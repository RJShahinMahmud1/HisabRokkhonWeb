...
export function NewsfeedView() {
  ...
  const t = {
    whatsOnYourMind: lang === 'hi' ? 'आपके दिमाग में क्या है?' : lang === 'bn' ? 'আপনার মনে কি আছে?' : 'What\'s on your mind?',
    post: lang === 'hi' ? 'पोस्ट करें' : lang === 'bn' ? 'পোস্ট করুন' : 'Post',
    deleteConfirm: lang === 'hi' ? 'क्या आप इस पोस्ट को हटाना चाहते हैं?' : lang === 'bn' ? 'পোস্টটি মুছে ফেলতে চান?' : 'Want to delete this post?',
    noPosts: lang === 'hi' ? 'कोई पोस्ट नहीं है' : lang === 'bn' ? 'কোনো पोस्ट নেই' : 'No posts yet',
    like: lang === 'hi' ? 'पसंद' : lang === 'bn' ? 'লাইক' : 'Like',
    comment: lang === 'hi' ? 'टिप्पणी' : lang === 'bn' ? 'কমেন্ট' : 'Comment',
    loading: lang === 'hi' ? 'लोड हो रहा है...' : lang === 'bn' ? 'লোড হচ্ছে...' : 'Loading...',
  };
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      {/* Create Post */}
      <div className="bg-white dark:bg-slate-950 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
        <form onSubmit={handleCreatePost} className="flex flex-col gap-3">
          <textarea
            value={newPostContent}
            onChange={(e) => setNewPostContent(e.target.value)}
            placeholder={t.whatsOnYourMind}
            className="w-full bg-slate-50 dark:bg-slate-900 rounded-xl px-4 py-3 border-none outline-none focus:ring-2 focus:ring-blue-500/50 resize-none text-slate-900 dark:text-slate-100 placeholder-slate-400"
            rows={2}
          ></textarea>
          <div className="flex justify-end">
             <button disabled={!newPostContent.trim()} className="bg-blue-600 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-blue-700 transition disabled:opacity-50">
               <span>{t.post}</span>
      ...
      <div className="space-y-4">
        ...
        {!loading && posts.map(post => {
          ...
          const timeStr = post.createdAt ? new Date(post.createdAt.toMillis()).toLocaleString(lang === 'hi' ? 'hi-IN' : lang === 'bn' ? 'bn-BD' : 'en-US', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }) : 'Just now';

          return (
            <div key={post.id} className="bg-white dark:bg-slate-950 p-4 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 space-y-3">
               ...
               <div className="flex items-center justify-between text-sm text-slate-500 border-t border-slate-100 dark:border-slate-800 pt-3">
               ...
               <div className="flex gap-2 border-t border-slate-100 dark:border-slate-800 pt-2">
                   <div className="relative group flex-1">
                       ...
                       <button 
                           ...
                           className={`w-full flex justify-center items-center gap-2 py-2 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-lg transition font-semibold ${userReactions[post.id] === 'love' ? 'text-rose-500' : userReactions[post.id] ? 'text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'}`}
                       >
                           {userReactions[post.id] === 'like' && <span className="text-xl">👍</span>}
                           {userReactions[post.id] === 'love' && <span className="text-xl">❤️</span>}
                           {userReactions[post.id] === 'wow' && <span className="text-xl">😲</span>}
                           {userReactions[post.id] === 'haha' && <span className="text-xl">😂</span>}
                           {userReactions[post.id] === 'angry' && <span className="text-xl">😡</span>}
                           {!userReactions[post.id] && <Heart className="w-5 h-5" />}
                   ...
                   <button 
                       ...
                       className="flex-1 flex justify-center items-center gap-2 py-2 hover:bg-slate-50 dark:hover:bg-slate-900 rounded-lg transition font-semibold text-slate-600 dark:text-slate-400"
                   >
                       <MessageSquare className="w-5 h-5" /> {t.comment}
                   </button>
               </div>
